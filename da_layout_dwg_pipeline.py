#!/usr/bin/env python3
"""
Design Automation — split a host DWG into **one DWG per paper layout** (zip: ``layout_dwgs.zip``).

Uses the same AppBundle as LayerPdfExport but a different AcCore script (``run_layout_dwgs.scr``)
that runs command **ExportAllLayoutDwgs**. Register a **separate Activity** whose ``commandLine``
uses ``\\Contents\\run_layout_dwgs.scr`` and whose ``ResultZip`` ``localName`` is ``layout_dwgs.zip``
(see ``da_register_batch.py --also-layout-dwg-activity``).
...
LOG = logging.getLogger("da_layout_dwg_pipeline")

DEFAULT_DA_LAYOUT_DWG_ACTIVITY_ID: str = os.environ.get(
    "DA_LAYOUT_DWG_ACTIVITY_ID", ""
).strip()
...
def create_workitem(
    ...
    *,
    ...
    auth = f"Bearer {token}"
    host_headers: dict[str, str] = {"Authorization": auth} if host_is_oss_urn else {}
...
def run_layout_dwg_split_pipeline(
    dwg_path: Path,
    out_zip: Path,
    aps_path: Path,
    activity_id: str,
    ...
    *,
    ...
    out_name = f"{Path(object_name).stem}_layout_dwgs.zip"
...
def main() -> int:
    p = argparse.ArgumentParser(
        description="Design Automation: one DWG per layout (ExportAllLayoutDwgs → layout_dwgs.zip)."
    )
    p.add_argument("--input", type=Path, required=True, help="Input .dwg")
    p.add_argument("--output", type=Path, required=True, help="Output .zip path")
    p.add_argument("--aps", type=Path, default=Path(".aps"))
    p.add_argument(
        "--activity-id",
        default=None,
        help="Layout-DWG split activity id (else DA_LAYOUT_DWG_ACTIVITY_ID env)",
    )
    ...
    act = (args.activity_id or DEFAULT_DA_LAYOUT_DWG_ACTIVITY_ID).strip()
    if not act:
        LOG.error(
            "Set --activity-id or DA_LAYOUT_DWG_ACTIVITY_ID "
            "(register with da_register_batch.py --also-layout-dwg-activity)."
        )
        return 1
